var turnBack = 8;			// cats will only walk on squares they were just on by 1/turnBack
...
function moveCat1Left(i){
	...
	if(oldcat1Location[i] == lookAheadTemp){		//move more likely to new spot
		var randomNum2 = Math.floor((Math.random()*turnBack)+1);
		if (randomNum2 != 1)
			return false;
	}
...
function moveCat3Left(i){
	...
	if(oldcat3Location[i] == lookAheadTemp){		//move more likely to new spot
		var randomNum3 = Math.floor((Math.random()*turnBack)+1);
		if (randomNum3 != 1)
			return false;
	}
